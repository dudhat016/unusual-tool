...
export const SYNTHESIZED_CATEGORIES: Record<string, CategorySeoEntry> = {
  'pdf-tools': {
    id: 'pdf-tools',
    slug: 'pdf-tools',
    name: 'PDF Tools Hub',
  ...
  'social-media-tools': {
    ...
    name: 'Social Media Generator Suite',
    h1: 'Free Social Media Post & Chat Mockup Generators',
    title: 'Social Media Mockup Generators - Instagram, WhatsApp, X & TikTok Mockups',
    metaDescription: 'Generate realistic social media post and chat mockups for Instagram, WhatsApp, X, TikTok, LinkedIn, and iMessage online with 2x HD PNG export.',
    description: 'Generate realistic social media post and chat mockups for Instagram, WhatsApp, X, TikTok, LinkedIn, and iMessage online with 2x HD PNG export.',
    quickAnswer: 'Create custom social media post and chat conversation mockups privately in your browser.',
    primaryKeyword: 'social media mockup generator',
    secondaryKeywords: ['fake instagram post generator', 'whatsapp chat mockup', 'fake tweet generator'],
    toolSlugs: ['instagram-post-generator', 'whatsapp-chat-generator', 'x-post-generator'],
    keyWorkflows: ['Create fake Instagram post mockup', 'Generate WhatsApp chat preview', 'Export 2x HD PNG social graphic'],
    faq: [
      { question: 'Are mockups free to download?', answer: 'Yes, 100% free with 2x High-DPI PNG export.' }
    ],
    indexable: true,
    matchingCategories: ['social', 'social-media-tools'],
  }
};

export const DEFAULT_CATEGORIES: CategorySeoEntry[] = Object.values(SYNTHESIZED_CATEGORIES);
...
export class DynamicCategoryService {
        ...
        (snapshot) => {
          if (!snapshot.empty) {
            ...
            if (snapshot.docs.length < 9) {
              this.seedAllCategoriesToFirestore().catch((err) => console.warn('Category seeding error:', err));
            }
          } else {
            this.seedAllCategoriesToFirestore().catch((err) => console.warn('Category seeding error:', err));
          }
  ...
  public static async seedAllCategoriesToFirestore(): Promise<number> {
    try {
      let count = 0;
      ...
      console.log(`Synced ${count} category documents to Firestore.`);
      return count;
    } catch (e) {
      console.error('Error seeding categories to Firestore:', e);
      throw e;
    }
  }
  ...
  public static getCategoryBySlug(slug: string): CategorySeoEntry | undefined {
    ...
    return SYNTHESIZED_CATEGORIES[targetId] || {
      id: targetId,
      slug: clean,
